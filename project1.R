...
x = read.table(text = grep("^[1,2]/2/2007", readLines(file), value = TRUE),
...
hist(x$Global_active_power,col="red",main=paste("Global Active Power"),
...
plot(sub$Sub_metering_1 ~ sub$Datetime, type = "l", 
     ylab = "Global Active Power (kilowatts)", xlab = "")
...
layout(matrix(c(1,2,3,4),2,2,byrow=FALSE))
    ...
    plot(sub$Sub_metering_1 ~ sub$Datetime, type = "l", ylab = "Energy sub metering",
         xlab = "")
    lines(sub$Sub_metering_2 ~ sub$Datetime, col = 'Red')
    lines(sub$Sub_metering_3 ~ sub$Datetime, col = 'Blue')
    ...
    plot(sub$Global_reactive_power ~ sub$Datetime, type = "l", 
